...
static void addEnPassMove(int m, MOVES_LIST &list) {
    MOVE t;
    t.move = m;
    // 1000000 + 105
    t.score = 1000105;
...
void generateAllMoves(BOARD *pos, MOVES_LIST &list, Globals &g) {
    ...
    if (side == WHITE) {
        ...
        for (int i = 0; i < pos->pieceNum[wP]; i++) {
            ...
            if (pos->enPass != NO_SQ) {
                if (sq + 9 == pos->enPass) {
                    addEnPassMove(MOVEI(sq, sq + 9, EMPTY, EMPTY, 0x40000), list);
                }
                if (sq + 11 == pos->enPass) {
                    addEnPassMove(MOVEI(sq, sq + 11, EMPTY, EMPTY, 0x40000), list);
                }
            }
        }
        // CASTLING
