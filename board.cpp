...
void parseFEN(const char * FEN, BOARD * pos, Globals g) {
    ...
    for (int i = 0; i < 4; i++) {
		...
		switch(*FEN) {
			case 'K': pos->castlePerm |= WHITE_K_K; break;
			case 'Q': pos->castlePerm |= WHITE_K_Q; break;
			case 'k': pos->castlePerm |= BLACK_K_K; break;
			case 'q': pos->castlePerm |= BLACK_K_Q; break;
			default:	     break;
        }
		FEN++;
	}
...
void UpdateMaterialLists(BOARD * pos, Globals g) {
    for (int i = 0; i < 120; i++) {
        ...
        if (piece != OFFBOARD && piece != EMPTY) {
            ...
            if (g.NonPawn[piece]) pos -> nonPawn[color]++;
            if (g.RookQueen[piece]) pos -> RQ[color]++;
            if (g.BishopKnight[piece]) pos -> KB[color]++;
...
bool checkBoard(BOARD *pos, Globals g) {   
	...
	for(sq64 = 0; sq64 < 64; ++sq64) {
		...
		if( g.NonPawn[t_piece] == true) _notPawn[colour]++;
		if( g.BishopKnight[t_piece] == true) _knightBishop[colour]++;
		if( g.RookQueen[t_piece] == true) _rookQueen[colour]++;
	...
	ASSERT(pos->enPass==NO_SQ || ( g.RowBoard[pos->enPass]==ROW_6 && pos->sideToMove == WHITE)
		 || ( g.RowBoard[pos->enPass]==ROW_3 && pos->sideToMove == BLACK));
