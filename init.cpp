...
void initEvalMasks(Globals &a)
{
	...
	for (int i = 0; i < 64; i++)
	{
		a.isolatedMask[i] = 0ULL;
		a.whitePassedMask[i] = 0ULL;
		a.blackPassedMask[i] = 0ULL;
	}
	...
	for (int sq = 0; sq < 64; sq++)
	{
		...
		while (tempSq < 64)
		{
			a.whitePassedMask[tempSq] |= (1ULL << tempSq);
			tempSq += 8;
		}
		...
		while (tempSq >= 0)
		{
			a.blackPassedMask[tempSq] |= (1ULL << tempSq);
			tempSq -= 8;
		}

		// Adds To Left Of Square
		if (a.ColumnBoard[a.SQ120(sq)] > COL_A)
		{
			// Note: Isolated Mask Checks if Entire Column Has No Pawns
			a.isolatedMask[sq] |= a.colBBMask[a.ColumnBoard[a.SQ120(sq)] - 1];

			tempSq = sq + 7;
			while (tempSq < 64)
			{
				a.whitePassedMask[sq] |= (1ULL << tempSq);
				tempSq += 8;
			}
			...
			while (tempSq >= 0)
			{
				a.blackPassedMask[sq] |= (1ULL << tempSq);
				tempSq -= 8;
			}
		}

		// Adds To Right of Sq
		if (a.ColumnBoard[a.SQ120(sq)] < COL_H)
		{
			a.isolatedMask[sq] |= a.colBBMask[a.ColumnBoard[a.SQ120(sq)] + 1];
